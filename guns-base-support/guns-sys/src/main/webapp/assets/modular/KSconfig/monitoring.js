layui.use(['layer', 'form', 'table', 'admin', 'ax', 'func'], function () {
    ...
    Role.initColumn = function () {
        return [[
           /* {type: 'checkbox'},*/
            {field: 'deveice', align: "center", sort: true, title: '设备编号'},
            {field: 'des', align: "center", sort: true, title: '状态'},
    ...
    Role.search = function () {
        var queryData = {};
        queryData['condition'] = $("#userName").val();
    ...
    Role.onEditRole = function (data) {
        func.open({
            height: 470,
            title: '修改账号',
            content: Feng.ctxPath + "/account/account_edit?id=" + data.id,
            tableId: Role.tableId
        });
    };
    ...
    Role.onDeleteRole = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/account/delete", function () {
                Feng.success("删除成功!");
                table.reload(Role.tableId);
            }, function (data) {
        ...
        Feng.confirm("是否删除账号 " + data.user_name + "?", operation);
    };
    ...
    var tableResult = table.render({
        elem: '#' + Role.tableId,
        url: Feng.ctxPath + '/KSconfig/list',
        page: true,
        height: "full-98",
        cellMinWidth: 100,
